...
export default function CartPopup({ isOpen, onClose }: CartPopupProps) {
  const { cartItems, totalPrice, removeFromCart, increaseQuantity, decreaseQuantity } =
    useCartStore();
  ...
  return (
    <Sheet open={isOpen} onOpenChange={handleClose}>
      <SheetContent
        className={`bg-gradient-to-b from-[#FEFAE1] to-[#daa400] w-full max-w-[400px] p-6 overflow-y-auto transform transition-all ease ${
          isClosing ? "translate-x-full" : "translate-x-0"
        } sm:max-w-[350px] md:max-w-[450px] lg:max-w-[500px]`}
      >
        <SheetTitle className="sr-only">Varukorg</SheetTitle>
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-bold">Översikt</h2>
        </div>

        {/* Lista över varukorgsobjekt */}
        <div data-cy="cart-item" className="space-y-4">
          {cartItems.map((item) => (
            <div
              key={item.id}
            ...
            >
              ...
              <div className="flex flex-col items-start ml-10 flex-1 min-w-0">
                <h3 data-cy="product-title" className="font-semibold text-sm md:text-base">
                  {item.title}
                </h3>
                <div className="flex items-center space-x-2 mt-2">
                  <p
                    data-cy="product-quantity"
                    className="text-gray-900 text-xs md:text-base w-24"
                  >
                    Antal:
                  </p>
                  <button
                  ...
                  <p data-cy="quantity" className="text-sm md:text-base">
                    {item.quantity}
                  </p>
                  <button
        ...
        {/* Sammanfattning och gå till kassan */}
        <div className="mt-6">
